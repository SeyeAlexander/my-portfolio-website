...
const ProjectCard: React.FC<ProjectCardProps> = ({ title, description, tags, link }) => {
  return (
    <div className='group relative grid gap-4 pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:opacity-100! lg:group-hover/list:opacity-50'>
      <div className='absolute -inset-x-4 -inset-y-4 z-0 hidden rounded-3xl transition motion-reduce:transition-none lg:-inset-x-6 lg:block lg:group-hover:bg-[#D1C5A7]/20 lg:group-hover:backdrop-blur-sm dark:lg:group-hover:bg-slate-800/35 lg:group-hover:shadow-sm dark:lg:group-hover:shadow-[inset_0_1px_0_0_rgba(148,163,184,0.1)] dark:lg:group-hover:drop-shadow-lg'></div>
